...
from model6_for_patchmatch import DualCNN
...
print_freq=200
save_freq=5000
check_point_dir="./checkpoint/model5_for_patchmatch/sr/train_by_bsd200_scale2"
...
netS,netD,endpoints=DualCNN(x)

y_struct=endpoints["compS"].outputs
y_detail=endpoints["compD"].outputs
cost=0
cost=cost+compute_charbonnier_loss(ys,y_struct, is_mean=True)
cost=cost+compute_charbonnier_loss(yd,y_detail, is_mean=True)
...
with tf.Session() as sess:
    ...
    for epoch in range(num_epoch):
        ...
        train_loss,y_s,y_d,_=sess.run([cost,y_struct,y_detail,train_op],
                                     feed_dict={x:img_lr, y:img_hr,
                                                ys:img_hr, yd:img_detail})
        mse, psnr=sr_metric(img_hr,y_s)
