...
def DetailNet(input_data):
     ...
     for i in range(16):
         ...
         out=tl.layers.Conv2d(out,n_filter=64,filter_size=(3,3), strides=(1,1),
                              act=tf.nn.relu,W_init=W_init,name="netD_conv{}".format(i+1))
         net.outputs=tf.add(net.outputs,out.outputs)
     net=tl.layers.Conv2d(net,n_filter=32,filter_size=(1,1), strides=(1,1),
...
def DualCNN(x,crop):
    ...
    netD.outputs=tf.image.resize_images(netD.outputs,[x.shape[1],x.shape[2]])
    net=tl.layers.ElementwiseLayer([netD, netS], tf.add,name="sumDS")
    net=tl.layers.Conv2d(net,n_filter=64,filter_size=(1,1), strides=(1,1),
                        act=tf.nn.relu,W_init=W_init,name="net_convout1")
    net=tl.layers.Conv2d(net,n_filter=32,filter_size=(1,1), strides=(1,1),
                        act=tf.nn.relu,W_init=W_init,name="net_convout1")
    net=tl.layers.Conv2d(net,n_filter=3,filter_size=(1,1), strides=(1,1),
                        act=tf.nn.relu,W_init=W_init,name="net_convout1")
