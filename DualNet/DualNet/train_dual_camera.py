...
from model_for_dual_camera import DualCNN,PatchCNN
...
batch_size=10
train_img_size=42
crop_size=int((train_img_size+3-1)/3)
lr0=0.0001
print_freq=200
save_freq=5000
check_point_dir="./checkpoint/dual_camera"
...
netD,netS,endpoints=DualCNN(x,crop)

y1_struct=endpoints["compS"].outputs
y1_detail=endpoints["compD"].outputs

x2=tf.placeholder(tf.float32,shape=[None,None,None,512],name="x2")
patch=tf.placeholder(tf.float32,shape=[None,None,None,512],name="patch")
y=tf.placeholder(tf.float32,shape=[None,None,None,3],name="y")

net=PatchCNN(x2,patch)
y2_out=net.outputs



cost=compute_charbonnier_loss(y,y2_out,is_mean=True)
...
with tf.Session() as sess:
    ...
    for epoch in range(num_epoch):
        img_lr, img_crop,img_hr=data_iter.creat3()
        y_s,y_d=sess.run([y1_struct,y1_detail],feed_dict={x:img_lr,crop:img_crop })
        y_patch=patch_match(y_s,y_d)
        train_loss,y_out=sess.run([cost,y2_out],feed_dict={x2:y_s,patch:y_patch,y:img_hr})
        mse, psnr=sr_metric(img_hr,y_out)
